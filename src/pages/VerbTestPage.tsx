...
export const VerbTestPage: React.FC = () => {
    ...
    const prepareOptions = (targetSentence: Sentence, sourcePool: Sentence[]) => {
        ...
        const pool = Array.from(new Set(sourcePool
            .filter(s => s.targetWord !== targetSentence.targetWord)
            .map(s => s.targetWord)));
        ...
        options.push(targetSentence.targetWord);
    ...
    const handleNext = () => {
        ...
        if (currentIndex + 1 < sentences.length) {
            ...
            setCurrentOptions(prepareOptions(sentences[currentIndex + 1], allSentences));
        } else {
            setIsFinished(true);
        }
    };
    ...
    const handleRetryIncorrect = () => {
        ...
        if (incorrectSentences.length > 0) {
            ...
            setCurrentIndex(0);
            setCurrentOptions(prepareOptions(incorrectSentences[0], allSentences));
            setCorrectCount(0);
            setIncorrectCount(0);
            setIncorrectIDs([]);
            setIsFinished(false);
    ...
    const handleStartShortQuiz = (count: number) => {
        ...
        if (selected.length > 0) {
            setCurrentOptions(prepareOptions(selected[0], allSentences));
        }
        setCorrectCount(0);
        setIncorrectCount(0);
        setIncorrectIDs([]);
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="text-center mb-8">
                                        ...
                                        <p className="text-lg text-gray-600">
                                            {sentences[currentIndex].targetWordTranslation}
                                        </p>
                                    </div>
