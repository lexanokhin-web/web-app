...
import type { BookSection, BookExercise, BookExerciseItem } from './useA2BookData';
...
export const useA2Quiz = () => {
    ...
    const startQuiz = useCallback(async () => {
        ...
        try {
            // 1. Fetch Index
            const indexRes = await fetch('/data/a2_book/index.json');
            ...
            // If less than 10 chapters available (e.g. only 8), take all of them shuffling
            ...
            for (let i = 0; i < selectedChapters.length; i++) {
                ...
                const chapterUrl = `/data/a2_book/${chapterInfo.file}`;
            ...
            if (loadedQuestions.length === 0) {
                // Determine if failure was due to missing files (likely in early dev)
                throw new Error('Could not load any questions. Some chapters might be missing content.');
            }
